...
latest_add_ts = int(datetime.datetime(2017, 11, 1).timestamp()) * 1000
pids = set()
...
def process_playlist(playlist):
    ...
    tassert(
        len(playlist["tracks"]) >= min_tracks_per_playlist,
        "min tracks per playlist < %d",
        min_tracks_per_playlist,
    )

    for field in playlist:
        tassert(
            field in required_fields or field in optional_fields,
            "extra field %s",
            field,
        )
    ...
    for i, track in enumerate(playlist["tracks"]):
        ...
        tassert(
            ...
            "mismatch artist name %s %s",
            track["artist_name"],
            artist_names[track["artist_uri"]],
        )
        ...
        tassert(
            ...
            "mismatch album name %s %s",
            track["album_name"],
            album_names[track["album_uri"]],
        )
        ...
        tassert(
            ...
            "mismatch track name %s %s",
            track["track_name"],
            track_names[track["track_uri"]],
        )

        tassert(
            is_track_uri(track["track_uri"]), "invalid track uri %s", track["track_uri"]
        )
        tassert(
            is_album_uri(track["album_uri"]), "invalid album uri %s", track["album_uri"]
        )
        tassert(
            is_artist_uri(track["artist_uri"]),
            "invalid artst uri %s",
            track["artist_uri"],
        )

    tassert(
        len(artists) >= min_artists_per_playlist, "too few artists %d", len(artists)
    )
    tassert(len(albums) >= min_albums_per_playlist, "too few albums %d", len(albums))
    tassert(
        len(albums) == playlist["num_albums"],
        "nalbum mismatch %d %d",
        len(albums),
        playlist["num_albums"],
    )
    tassert(
        len(artists) == playlist["num_artists"],
        "nartist mismatch %d %d",
        len(artists),
        playlist["num_artists"],
    )
    tassert(
        len(playlist["tracks"]) == playlist["num_tracks"],
        "ntracks mismatch %d %d",
        len(playlist["tracks"]),
        playlist["num_tracks"],
    )
    tassert(
        total_duration == playlist["duration_ms"],
        "mismiatch duration %d %d",
        total_duration,
